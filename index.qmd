...
```{r working datasets}
micha_subset <- posmo_micha_truth %>% filter(datetime < ymd("20230507") & datetime > ymd("20230506"))
...
#working_dataset <- working_dataset_pre_cleaned
...
working_dataset_static <- working_dataset
working_dataset <- filterStaticByAngle(working_dataset, 60)
...
```{r}
working_dataset_line <- point2line(working_dataset_static)

working_dataset_line_cleaned <- point2line(working_dataset)
  
mapview(working_dataset, zcol="angle", alpha.regions=1.0) +
  mapview(working_dataset_line, color="#901010", lwd=2) +
  mapview(working_dataset_line_cleaned, color="green", lwd=2) +
  mapview(working_dataset_pre_cleaned, zcol="angle")
```

```{r filter static points using static points}
filter_staying_points <- function(data, threshold_distance, consecutive_points) {
  require(geosphere)
  
  # transform to WGS84
  data <- data %>% st_transform(4326)
  
  # Extract longitude and latitude from the geometry attribute
  coords <- data.frame(st_coordinates(data))
  data$longitude <- coords$X
  data$latitude <- coords$Y
  
  # Calculate distances to the next ten points
  ...
  return(list(filtered_data = filtered_data, distances = distances)) # distances just needed for testing thresholds
}

```

```{r}
result <- filter_staying_points(working_dataset, threshold_distance = 60, consecutive_points = 5)
filtered_data <- result$filtered_data
distances <- result$distances
working_dataset$distances <- distances

filtered_data_line <- point2line(filtered_data)

color_palette <- colorRampPalette(c("cyan", "blue"))
color_palette2 <- colorRampPalette(c("yellow", "red"))
n_values <- 

mapview(working_dataset, zcol="distances", alpha.regions=1.0, col.regions = color_palette2(length(unique(working_dataset$distances)))) +
  mapview(filtered_data, zcol="distances", alpha.regions=1.0, col.regions = color_palette(length(unique(filtered_data$distances)))) +
  mapview(filtered_data_line, color="#901010", lwd=2) 
  
```


```{r filter static points using angle}
