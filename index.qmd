...
combined_df <- rbind(posmo_micha_truth_csv, posmo_cyril_truth_csv)
```
...
```{r combine data}
# run Preprocessing
result <- filterStaticByDistance(posmo_micha_truth, threshold_distance = 60, consecutive_points = 5)
result$filtered_data
filtered_by_angle <- filterStaticByAngle(result$filtered_data, 60)
working_dataset <- calc_movement_params(filtered_by_angle)

result <- filterStaticByDistance(posmo_cyril_truth, threshold_distance = 60, consecutive_points = 5)
result$filtered_data
filtered_by_angle <- filterStaticByAngle(result$filtered_data, 60)

# Combine datasets
working_dataset <- rbind(working_dataset,calc_movement_params(filtered_by_angle))
...
```{r}
# Combine data
#working_dataset <- rbind(posmo_micha_truth_csv, posmo_cyril_truth_csv)
```
...
standardized <- working_dataset_copy[, 5:14] %>% 
  st_drop_geometry() %>%
...
write.csv(working_dataset, file = "data/full_working_dataset.csv", row.names = FALSE)
```
