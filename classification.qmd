...
If we categorize only Bike, Bus, Car Train, Tram, Walk & Others we found the best model with C = 1. The accuracy lies at 81.9%
...
model.final <- predict(model.svmRadial.cv, working_dataset_full_copy)

# Create final df
working_dataset_result <- data.frame(working_dataset_full_copy, model.final) 

# Confusion Matrix
# Note: Only works if result and working dataset have identical transport modes as factors.
(conf_matrix <- confusionMatrix( as.factor(working_dataset_result$model.final),as.factor(working_dataset_result$transport_mode)))
...
Precision: Precision measures the proportion of correctly predicted positive instances out of the total instances predicted as positive. It is useful when the focus is on minimizing false positives.

Recall (Sensitivity or True Positive Rate): Recall measures the proportion of correctly predicted positive instances out of the total actual positive instances. It is useful when the goal is to minimize false negatives.

F1-Score: The F1-score combines precision and recall into a single metric. It provides a balance between precision and recall and is useful when both false positives and false negatives are important.
...
single_point_correction <- function(df, x, threshold_percentage, iterations) {
  ...
  print(conf_matrix_func <- confusionMatrix(as.factor(df$transport_mode), as.factor(df$model.final)))
  # Precision for each class
  print(precision_func <- conf_matrix_func$byClass[, "Precision"])
  # Recall for each class
  print(recall_func <- conf_matrix_func$byClass[, "Recall"])
  # F1-Score for each class
  print(f1_score_func <- conf_matrix_func$byClass[, "F1"])
...
working_dataset_result <- single_point_correction(working_dataset_result, 10, 0.65, 5)
